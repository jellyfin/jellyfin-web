...
class CastPlayer {
    ...
    session ?: {
        ...
        media: any[];
        addMessageListener: (namespace: string, listener: (namespace: string, message: any) => void) => void;
        addMediaListener: (listener: (mediaSession: any) => void) => void;
        addUpdateListener: (listener: (isAlive: boolean) => void) => void;
        stop: (successCallback: (message: string) => void, errorCallback: (error: any) => void) => void;
        sendMessage: (namespace: string, message: string, successCallback: () => void, errorCallback: (error: any) => void) => void;
    ...
    mediaStatusUpdateHandler: (e: any) => void;
    constructor() {
        /* device variables */
    ...
    messageListener(namespace: string, message: any) {
     ...
     * @returns Promise
     */
    loadMedia(options: {items: any[]}, command: string) {
        if (!this.session) {
    ...
    sendMessageInternal(message: any) {
...
function alertText(text: string, title: string) {
    alert({
        text,
        title
    });
...
function getItemsForPlayback(apiClient: any, query: any) {
    const userId = apiClient.getCurrentUserId();
 ...
 */
function bindEventForRelay(instance: any, eventName: string) {
...
function initializeChromecast(this: any) {
    const instance = this;
...
export default class ChromecastPlayer implements Plugin {
    name: string;
    type: PluginType;
    id: string;
    ...
    _playNextAfterEnded: boolean = true; // playbackManager needs this
    _currentResolve: (() => void) | null = null;
    _currentReject: (() => void) | null = null;
    ...
    playWithCommand(options: any, command: string) {
        if (!options.items) {
    ...
    setMaxStreamingBitrate(options: any) {
        this._castPlayer.sendMessage({
            options: options,
    ...
    canQueueMediaType(mediaType?: string) {
    ...
    clearQueue(_currentTime?: number) {
        // not supported yet
    }
}
