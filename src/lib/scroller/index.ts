 ...
 * @return {Void}
 */
function disableOneEvent(event: Event) {
...
function within(number: number, num1: number, num2?: number) {
...
const scrollerFactory = function (frame: HTMLElement, options: any) {
    // Extend options
    ...
    function sibling(n: Node, elem?: Node) {
        const matched = [];
    ...
    function nativeScrollTo(container: HTMLElement, pos: number, immediate?: boolean) {
        if (container.scroll) {
            if (o.horizontal) {
                container.scroll({
                    left: pos,
    ...
    /**
         ...
         * @return {Void}
         */
    self.slideTo = function (newPos: number, immediate?: boolean, fullItemPos?: any) {
        ensureSizeInfo();
        const pos = self._pos;
    ...
    function setStyleProperty(elem: HTMLElement, name: string, value: string, speed: number, resetTransition?: boolean) {
