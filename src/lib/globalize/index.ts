...
export function translate(...args: any[]) {
    let val = translateKey(args);
    for (let i = 1; i < args.length; i++) {
        val = val.replaceAll('{' + (i - 1) + '}', args[i].toLocaleString(currentCulture));
    }
    return val;
}

export function translateHtml(html, module ?: any) {
...
let _defaultModule;
export function defaultModule(val: string) {
    if (val) {
        _defaultModule = val;
    }
...
Events.on(userSettings, 'change', (_e, name) => {
