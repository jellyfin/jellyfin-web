...
function loadTranslation(translations, lang) {
    ...
    let filtered = translations.filter((t) => normalizeLocaleName(t.lang) === lang);

    if (!filtered.length) {
        lang = lang.replace(/-.*/, '');

        filtered = translations.filter((t) => normalizeLocaleName(t.lang) === lang);

        if (!filtered.length) {
            filtered = translations.filter((t) => normalizeLocaleName(t.lang) === FALLBACK_CULTURE);
        }
    }
