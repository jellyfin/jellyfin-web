...
import './jellyfin-select.scss';
import 'webcomponents.js/webcomponents-lite';

/* eslint-disable indent */

    const JellyfinSelectPrototype = Object.create(HTMLSelectElement.prototype);
    ...
    JellyfinSelectPrototype.createdCallback = function () {
        if (!this.id) {
            this.id = 'jellyfinselect' + inputId;
            inputId++;
        }

        this.classList.add('jellyfin-select-withcolor');

        if (layoutManager.tv) {
            this.classList.add('jellyfin-select-focusscale');
        }
    ...
    JellyfinSelectPrototype.attachedCallback = function () {
        if (this.classList.contains('jellyfin-select')) {
            return;
        }

        this.classList.add('jellyfin-select');
        ...
        if (this.classList.contains('jellyfin-select-withcolor')) {
    ...
    JellyfinSelectPrototype.setLabel = function (text) {
        const label = this.parentNode.querySelector('label');

        label.innerHTML = text;
    };

    document.registerElement('jellyfin-select', {
        prototype: JellyfinSelectPrototype,
        extends: 'select'
    });
