import './jellyfin-scrollbuttons.scss';
import 'webcomponents.js/webcomponents-lite';
import '../jellyfin-button/paper-icon-button-light';

/* eslint-disable indent */

const JellyfinScrollButtonsPrototype = Object.create(HTMLDivElement.prototype);

    JellyfinScrollButtonsPrototype.createdCallback = function () {};

    function getScrollButtonHtml(direction) {
        let html = '';
        const icon = direction === 'left' ? 'chevron_left' : 'chevron_right';

        html += '<button type="button" is="paper-icon-button-light" data-ripple="false" data-direction="' + direction + '" class="jellyfin-scrollbuttons-button">';
    ...
    JellyfinScrollButtonsPrototype.attachedCallback = function () {
        const scroller = this.nextSibling;
        this.scroller = scroller;

        const parent = this.parentNode;
        parent.classList.add('jellyfin-scroller-container');

        this.innerHTML = getScrollButtonHtml('left') + getScrollButtonHtml('right');

        const buttons = this.querySelectorAll('.jellyfin-scrollbuttons-button');
    ...
    JellyfinScrollButtonsPrototype.detachedCallback = function () {
    ...
    document.registerElement('jellyfin-scrollbuttons', {
        prototype: JellyfinScrollButtonsPrototype,
        extends: 'div'
    });
