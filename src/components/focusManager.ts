...
function autoFocus(view: HTMLElement, defaultToFirst ?: boolean, findAutoFocusElement ?: boolean) {
    let element;
...
function isFocusable(elem: HTMLElement) {
    return focusableTagNames.indexOf(elem.tagName) !== -1
            || (elem.classList?.contains('focusable'));
}

function normalizeFocusable(elem: HTMLElement, originalElement: HTMLElement) {
    if (elem) {
...
function focusableParent(elem: HTMLElement) {
...
function isCurrentlyFocusableInternal(elem: HTMLElement) {
    // http://stackoverflow.com/questions/19669786/check-if-element-is-visible-in-dom
    return elem.offsetParent !== null;
}

// Determines if a focusable element can be focused at a given point in time
function isCurrentlyFocusable(elem: HTMLElement) {
...
function getFocusableElements(parent: HTMLElement, limit, excludeClass) {
...
function isFocusContainer(elem: HTMLElement, direction: number) {
...
function getFocusContainer(elem: HTMLElement, direction: number) {
...
function nav(activeElement: HTMLElement, direction: number, container: HTMLElement, focusableElements: HTMLElement[] | null) {
...
function intersectsInternal(a1: number, a2: number, b1: number, b2: number) {
...
function focusFirst(container: HTMLElement, focusableSelector: string) {
...
function focusLast(container: HTMLElement, focusableSelector: string) {
...
function moveFocus(sourceElement: HTMLElement, container: HTMLElement, focusableSelector: string, offset: number) {
    const elems = container.querySelectorAll(focusableSelector);
    const list = [];
    let i;
    let length;
