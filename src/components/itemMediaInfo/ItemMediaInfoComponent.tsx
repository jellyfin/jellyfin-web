...
const ItemMediaInfoComponent: React.FC<ItemMediaInfoComponentProps> = ({ user, item }) => {
    ...
    const renderMediaSource = (version: any, index: number) => {
        ...
        if (version.Container) {
            attributes.push(
                renderAttribute(globalize.translate('MediaInfoContainer'), version.Container)
            );
        }
        if (version.Formats?.length) {
            attributes.push(
                renderAttribute(globalize.translate('MediaInfoFormat'), version.Formats.join(','))
            );
        }
        if (version.Path && user?.Policy.IsAdministrator) {
            attributes.push(
                renderAttribute(globalize.translate('MediaInfoPath'), version.Path, true)
            );
        }
        if (version.Size) {
            attributes.push(
                renderAttribute(globalize.translate('MediaInfoSize'), getReadableSize(version.Size))
            );
        }
        ...
        return (
            <div key={version.Id || index} className="mediaInfoSource">
                ...
                {streams.map((stream, sIndex) => {
                    ...
                    if (stream.Profile) {
                        streamAttributes.push(
                            renderAttribute(globalize.translate('MediaInfoProfile'), stream.Profile)
                        );
                    }
                    if (stream.Level > 0) {
                        streamAttributes.push(
                            renderAttribute(globalize.translate('MediaInfoLevel'), stream.Level)
                        );
                    }
                    ...
                    if (stream.NalLengthSize)
                        streamAttributes.push(renderAttribute('NAL', stream.NalLengthSize));
        ...
        <div id="mediaInfoContent">
            {item.MediaSources.map((version: any, index: number) => (
                <React.Fragment key={version.Id || index}>
                    {index > 0 && (
                        <div style={{ borderTop: '1px solid #444', margin: '1em 0' }}></div>
                    )}
