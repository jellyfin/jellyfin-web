...
class CrossfadeImageIntegration {
    ...
    async preloadForCrossfade(options: CrossfadePreloadOptions): Promise<boolean> {
        ...
        if (imageUrl) {
            imagePromises.push(imagePreloader.preloadImage(imageUrl).then(() => {}));
        }

        if (backdropUrl) {
            imagePromises.push(imagePreloader.preloadImage(backdropUrl).then(() => {}));
        }

        if (artistLogoUrl) {
            imagePromises.push(imagePreloader.preloadImage(artistLogoUrl).then(() => {}));
        }

        if (discImageUrl) {
            imagePromises.push(imagePreloader.preloadImage(discImageUrl).then(() => {}));
        }
