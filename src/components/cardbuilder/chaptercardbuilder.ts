...
function buildChapterCard(item: any, apiClient: any, chapter: any, index: number, options: ChapterOptions, className: string, shape: string): string {
    ...
    if (!imgUrl) cardImageContainer += '<span class="material-icons cardImageIcon local_movies" aria-hidden="true"></span>';
...
export function buildChapterCards(item: any, chapters: any[], options: ChapterOptions): void {
    if (options.parentContainer) {
        ...
        else { options.parentContainer.classList.add('hide'); return; }
    }
