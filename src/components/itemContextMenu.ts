...
import { BaseItemDto, UserDto } from '@jellyfin/client-axios';
...
interface ItemContextMenuOptions {
    cancelTimer: boolean;
    clearQueue: boolean;
    collectionId: string;
    deleteItem: boolean;
...
export function getCommands(options: ItemContextMenuOptions): (MenuEntry | MenuDivider)[] {
    ...
    const commands: (MenuEntry | MenuDivider)[] = [];
...
function executeCommand(item: BaseItemDto, id: string, { playlistId, collectionId }: { playlistId?: string, collectionId?: string }) {
        ...
        switch (id) {
            case 'addtocollection':
                import('./collectionEditor/collectionEditor').then(({default: CollectionEditor}) => {
                    ...
                    collectionEditor.show().then(getResolveFunction(resolve, id, true), getResolveFunction(resolve, id));
                });
                break;
            case 'addtoplaylist':
                import('./playlisteditor/playlisteditor').then(({default: PlaylistEditor}) => {
                    ...
                    playlistEditor.show().then(getResolveFunction(resolve, id, true), getResolveFunction(resolve, id));
                });
                break;
            case 'download':
            ...
            case 'editsubtitles':
                import('./subtitleeditor/subtitleeditor').then(({default: subtitleEditor}) => {
                    subtitleEditor.show(item.Id, serverId).then(getResolveFunction(resolve, id, true), getResolveFunction(resolve, id));
                });
                break;
            case 'edit':
            ...
            case 'identify':
                import('./itemidentifier/itemidentifier').then((itemIdentifier) => {
                    itemIdentifier.show(item.Id, serverId).then(getResolveFunction(resolve, id, true), getResolveFunction(resolve, id));
                });
                break;
            case 'moremediainfo':
                import('./itemMediaInfo/itemMediaInfo').then((itemMediaInfo) => {
                    itemMediaInfo.show(item.Id, serverId).then(getResolveFunction(resolve, id), getResolveFunction(resolve, id));
                });
                break;
            case 'refresh':
            ...
            case 'record':
                import('./recordingcreator/recordingcreator').then(({default: recordingCreator}) => {
                    recordingCreator.show(item.Id, serverId).then(getResolveFunction(resolve, id, true), getResolveFunction(resolve, id));
                });
                break;
            case 'shuffle':
            ...
            case 'removefromplaylist':
                apiClient.ajax({
                    url: apiClient.getUrl('Playlists/' + playlistId + '/Items', {
...
function deleteTimer(item: BaseItemDto, resolve, command: string) {
...
function deleteSeriesTimer(item: BaseItemDto, resolve, command: string) {
    import('./recordingcreator/recordinghelper').then(({default: recordingHelper}) => {
...
function play(item: BaseItemDto, resume: boolean, queue?: boolean, queueNext?: boolean): void {
...
function editItem(apiClient, item: BaseItemDto): Promise<unknown> {
    return new Promise(function (resolve, reject) {
...
function deleteItem(item: BaseItemDto): Promise<unknown> {
    return new Promise(function (resolve, reject) {
...
function refresh(apiClient, item: BaseItemDto): void {
...
export function show(options: ItemContextMenuOptions): Promise<unknown> {
