define(['dialogHelper', 'globalize', 'connectionManager', 'events', 'browser', 'require', 'emby-checkbox', 'emby-collapse', 'css!./style'], function (dialogHelper, globalize, connectionManager, events, browser, require) {
    ...
    function updateFilterControls (context, options) {
        ...
        for (var j = 0; j < elems.length; j++) {
            var chkVideoTypeFilter = elems[j];
    ...
    return function (options) {
        ...
        function bindEvents (context) {
            ...
            if (options.mode === 'livetvchannels') {
                ...
                elems = context.querySelectorAll('.chkStandardFilter');
                for (var j = 0; j < elems.length; j++) {
                    elems[j].addEventListener('change', onStandardFilterChange);
                }
            }
            elems = context.querySelectorAll('.chkVideoTypeFilter');
            for (var k = 0; k < elems.length; k++) {
                elems[k].addEventListener('change', onVideoTypeFilterChange);
            }
            ...
            context.addEventListener('change', function (e) {
                ...
                if (chkTagFilter) {
                    filterName = chkTagFilter.getAttribute('data-filter');
                    filters = query.Tags || '';
                    delimiter = '|';
                    filters = (delimiter + filters).replace(delimiter + filterName, '').substring(1);
                ...
                if (chkYearFilter) {
                    filterName = chkYearFilter.getAttribute('data-filter');
                    filters = query.Years || '';
                    delimiter = ',';
                    filters = (delimiter + filters).replace(delimiter + filterName, '').substring(1);
                ...
                if (chkOfficialRatingFilter) {
                    filterName = chkOfficialRatingFilter.getAttribute('data-filter');
