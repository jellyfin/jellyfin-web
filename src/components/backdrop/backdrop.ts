...
class Backdrop {
    ...
    load(url: string, parent: HTMLElement, existingBackdropImage: HTMLElement | null) {
        const img = new Image();
        const self = this;
...
export function clearBackdrop(clearAll ?: boolean) {
    clearRotation();
...
function getImageUrls(items: any[], imageOptions) {
    const list = [];
    const onImg = img => {
        list.push(img);
    };
...
export function setBackdrop(url: string, imageOptions) {
 ...
 */
export function setBackdropTransparency(level: keyof typeof TRANSPARENCY_LEVEL) {
