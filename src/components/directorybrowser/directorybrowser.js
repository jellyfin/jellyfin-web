        ...
        return apiClient.ajax({
            ...
            if (response) {
                ...
                if (response.status === 404) {
                    ...
                    return Promise.reject(Error('The path could not be found. Please ensure the path is valid and try again.'));
                }
                if (response.status === 500) {
                    if (validateWriteable) {
                        ...
                        return Promise.reject(Error('Jellyfin Server requires write access to this folder. Please ensure write access and try again.'))
                    } else {
                        alertText('The path could not be found. Please ensure the path is valid and try again.')
                        return Promise.reject(Error('The path could not be found. Please ensure the path is valid and try again.'))
                    }
