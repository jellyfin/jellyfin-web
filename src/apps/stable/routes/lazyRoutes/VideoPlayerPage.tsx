...
const VideoPlayerPage: FC = () => {
    ...
    const onShowVideoOsd = (e: Event, ...args: unknown[]) => {
        const isShowing = args[0] as boolean;
    ...
    useEffect(() => {
        ...
        if (doc) Events.on(doc, EventType.SHOW_VIDEO_OSD, onShowVideoOsd as any);

        return () => {
            if (doc) Events.off(doc, EventType.SHOW_VIDEO_OSD, onShowVideoOsd as any);
        };
    }, []);
