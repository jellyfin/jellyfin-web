...
const LiveTVRecordingsPage: React.FC = () => {
    ...
    const handleRecordingPlay = useCallback(async (recordingId: string, recordingName: string) => {
        try {
            ...
            logger.error('[LiveTVRecordingsPage] Failed to play recording', { recordingId, error: err });
            setPlayingRecordingId(null);
        }
    }, []);
    ...
    return (
        <Box style={{ padding: vars.spacing['5'] }}>
            <Heading.H2 style={{ marginBottom: vars.spacing['6'] }}>{globalize.translate('Recordings')}</Heading.H2>
                                        ...
                                        >
                                            <IconButton
                                                ...
                                                onClick={(e: React.MouseEvent) => {
                                                    e.stopPropagation();
                                                    handleRecordingPlay(item.recordingId, item.name);
                                                }}
