...
const LiveTVRecordingsPage: React.FC = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                setTimeout(() => {
                    setData([
                        { id: 1, name: 'Latest Recordings', count: 24 },
                        { id: 2, name: 'Movies', count: 12 },
                        { id: 3, name: 'TV Shows', count: 45 }
                    ]);
                    setIsLoading(false);
                }, 300);
            } catch (error) {
    ...
    return (
        <Box style={{ padding: vars.spacing.md }}>
            <Heading.H2 style={{ marginBottom: vars.spacing.lg }}>{globalize.translate('Recordings')}</Heading.H2>
