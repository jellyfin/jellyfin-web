...
function getDeviceHtml(device) {
    ...
    html +=
        '<button is="paper-icon-button-light" class="itemAction btnCardOptions autoSize" data-action="menu"><span class="material-icons more_vert" aria-hidden="true"></span></button>';
    html +=
        '<div class="cardText">' +
...
function renderDevices(page, devices) {
    page.querySelector('.devicesList').innerHTML = devices
        .map(getDeviceHtml)
        .join('');
}
...
function renderProviders(page, providers) {
    ...
    if (providers.length) {
        ...
        for (let i = 0, length = providers.length; i < length; i++) {
            ...
            html +=
                '<span class="listItemIcon material-icons dvr" aria-hidden="true"></span>';
            html += '<div class="listItemBody two-line">';
            html +=
                '<a is="emby-linkbutton" style="display:block;padding:0;margin:0;text-align:left;" class="clearLink" href="' +
            ...
            html += '</div>';
            html +=
                '<button type="button" is="paper-icon-button-light" class="btnOptions" data-id="' +
                provider.Id +
                '"><span class="material-icons listItemAside more_vert" aria-hidden="true"></span></button>';
            html += '</div>';
        }
...
function mapChannels(page, providerId) {
    import('components/channelMapper/channelMapper').then(
        ({ default: ChannelMapper }) => {
            new ChannelMapper({
            ...
            actionsheet
                ...
                .then(function (id) {
                    switch (id) {
                        case 'delete':
                            deleteDevice(
                                dom.parentWithClass(button, 'page'),
                                tunerDeviceId
                            );
                            break;
                        ...
                        case 'edit':
                            Dashboard.navigate(
                                'dashboard/livetv/tuner?id=' + tunerDeviceId
                            );
                    }
                });
        }
...
$(document)
    .on('pageinit', '#liveTvStatusPage', function () {
        const page = this;
        page.querySelector('.btnAddDevice').addEventListener(
            'click',
            function () {
                addDevice();
            }
        ...
        if (page.querySelector('.formAddDevice')) {
            // NOTE: unused?
            page.querySelector('.formAddDevice').addEventListener(
                'submit',
                function (e) {
                    e.preventDefault();
        ...
        page.querySelector('.btnAddProvider').addEventListener(
            'click',
            function () {
                addProvider(this);
            }
        );
        page.querySelector('.devicesList').addEventListener(
            'click',
            onDevicesListClick
        );
    })
