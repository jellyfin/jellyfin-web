...
function renderDevices(page: Document, devices) {
    page.querySelector('.devicesList').innerHTML = devices.map(getDeviceHtml).join('');
}

function deleteDevice(page: Document, id) {
...
function submitAddDeviceForm(page: Document) {
...
function renderProviders(page: Document, providers) {
    let html = '';
...
function showProviderOptions(page: Document, providerId, button) {
    const items = [];
    items.push({
...
function mapChannels(page: Document, providerId) {
...
function deleteProvider(page: Document, id) {
...
function getTunerName(providerId: string) {
...
function getProviderConfigurationUrl(providerId: string) {
    switch (providerId.toLowerCase()) {
        case 'xmltv':
...
function onDevicesListClick(e: Event) {
