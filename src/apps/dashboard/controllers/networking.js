        ...
        validateHttps(form).then(function () {
            ...
            ApiClient.getNamedConfiguration('network').then(function (config) {
                ...
                config.IsRemoteIPFilterBlacklist =
                    form.querySelector('#selectExternalAddressFilterMode')
                        .value === 'blacklist';
                config.PublicHttpPort =
                    form.querySelector('#txtPublicHttpPort').value;
                config.PublicHttpsPort = form.querySelector(
                    '#txtPublicHttpsPort'
                ).value;
                config.InternalHttpPort =
                    form.querySelector('#txtPortNumber').value;
                config.InternalHttpsPort =
                    form.querySelector('#txtHttpsPort').value;
                config.EnableHttps =
                    form.querySelector('#chkEnableHttps').checked;
                config.RequireHttps =
                    form.querySelector('#chkRequireHttps').checked;
                config.BaseUrl = form.querySelector('#txtBaseUrl').value;
                config.EnableRemoteAccess =
                    form.querySelector('#chkRemoteAccess').checked;
                config.CertificatePath =
                    form.querySelector('#txtCertificatePath').value || null;
                config.CertificatePassword =
                    form.querySelector('#txtCertPassword').value || null;
                config.AutoDiscovery =
                    form.querySelector('#chkAutodiscovery').checked;
...
export default function (view) {
    function loadPage(page, config) {
        ...
        page.querySelector('#txtPublicHttpsPort').value =
            config.PublicHttpsPort;
        page.querySelector('#txtLocalAddress').value = (
            config.LocalNetworkAddresses || []
        ).join(', ');
        page.querySelector('#txtLanNetworks').value = (
            config.LocalNetworkSubnets || []
        ).join(', ');
        page.querySelector('#txtKnownProxies').value = (
            config.KnownProxies || []
        ).join(', ');
        page.querySelector('#txtExternalAddressFilter').value = (
        ...
        page.querySelector('#txtCertPassword').value =
            config.CertificatePassword || '';
    ...
    view.querySelector('#chkRemoteAccess').addEventListener(
        'change',
        function () {
            if (this.checked) {
                view.querySelector(
                    '.fldExternalAddressFilter'
                ).classList.remove('hide');
                view.querySelector(
                    '.fldExternalAddressFilterMode'
                ).classList.remove('hide');
                view.querySelector('.fldPublicHttpPort').classList.remove(
                    'hide'
                );
                view.querySelector('.fldPublicHttpsPort').classList.remove(
                    'hide'
                );
            } else {
                view.querySelector('.fldExternalAddressFilter').classList.add(
                    'hide'
                );
                view.querySelector(
                    '.fldExternalAddressFilterMode'
    ...
    view.querySelector('#btnSelectCertPath').addEventListener(
        'click',
        function () {
    ...
    view.querySelector('.dashboardHostingForm').addEventListener(
        'submit',
        onSubmit
    );
