...
import { getDisplayPlayMethod } from 'components/playback/playmethodhelper';
...
function showPlaybackInfo(btn, session) {
    let title;
    const text = [];
    const displayPlayMethod = getDisplayPlayMethod(session);
...
function onActiveDevicesClick(evt: Event) {
...
function filterSessions(sessions: unknown[]) {
    const list = [];
...
function refreshActiveRecordings(view, apiClient: ApiClient) {
...
function reloadSystemInfo(view, apiClient: ApiClient) {
...
function pollForInfo(view, apiClient: ApiClient) {
