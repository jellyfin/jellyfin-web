...
function PluginDrawerSection(): React.ReactElement {
    ...
    useEffect(() => {
        if (error)
            logger.error('[PluginDrawerSection] unable to fetch plugin config pages', { error });
    }, [error]);
    ...
    return (
        <List
            ...
            subheader={
                <ListSubheader id="plugins-subheader">
            ...
            <ListItemLink
                ...
                excludePaths={
                    Array.isArray(pagesInfo)
                        ? pagesInfo.map((p) => `/${Dashboard.getPluginUrl(p.Name ?? '')}`)
                        : []
                }
            >
                <ListItemDecorator>
