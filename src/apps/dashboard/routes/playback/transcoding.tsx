...
export const action = async ({ request }: ActionFunctionArgs) => {
    ...
    config.EnableAudioVbr = data.EnableAudioVbr.toString() === 'on';
    config.DownMixAudioBoost = parseInt(data.DownMixAudioBoost.toString(), 10);
    config.DownMixStereoAlgorithm = data.DownMixStereoAlgorithm.toString() as DownMixStereoAlgorithms;
    config.MaxMuxingQueueSize = parseInt(data.MaxMuxingQueueSize.toString(), 10);
    config.TranscodingTempPath = data.TranscodingTempPath.toString();
    config.FallbackFontPath = data.FallbackFontPath.toString();
    config.EnableFallbackFont = data.EnableFallbackFont.toString() === 'on';
    config.EncodingThreadCount = parseInt(data.EncodingThreadCount.toString(), 10);
    config.HardwareAccelerationType = data.HardwareAccelerationType.toString() as HardwareAccelerationType;
    config.VaapiDevice = data.VaapiDevice.toString();
    config.QsvDevice = data.QsvDevice.toString();
    config.EnableTonemapping = data.EnableTonemapping.toString() === 'on';
    config.EnableVppTonemapping = data.EnableVppTonemapping.toString() === 'on';
    config.EnableVideoToolboxTonemapping = data.EnableVideoToolboxTonemapping.toString() === 'on';
    config.TonemappingAlgorithm = data.TonemappingAlgorithm.toString() as TonemappingAlgorithm;
    config.TonemappingMode = data.TonemappingMode.toString() as TonemappingMode;
    config.TonemappingRange = data.TonemappingRange.toString() as TonemappingRange;
    config.TonemappingDesat = parseFloat(data.TonemappingDesat.toString());
    config.TonemappingPeak = parseFloat(data.TonemappingPeak.toString());
    config.TonemappingParam = parseFloat(data.TonemappingParam.toString());
    config.VppTonemappingBrightness = parseFloat(data.VppTonemappingBrightness.toString());
    config.VppTonemappingContrast = parseFloat(data.VppTonemappingContrast.toString());
    config.EncoderPreset = data.EncoderPreset.toString() as EncoderPreset;
    config.H264Crf = parseInt(data.H264Crf.toString() || '0', 10);
    config.H265Crf = parseInt(data.H265Crf.toString() || '0', 10);
    config.DeinterlaceMethod = data.DeinterlaceMethod.toString() as DeinterlaceMethod;
    config.DeinterlaceDoubleRate = data.DeinterlaceDoubleRate.toString() === 'on';
    config.EnableSubtitleExtraction = data.EnableSubtitleExtraction.toString() === 'on';
    config.EnableThrottling = data.EnableThrottling.toString() === 'on';
    config.EnableSegmentDeletion = data.EnableSegmentDeletion.toString() === 'on';
    config.ThrottleDelaySeconds = parseInt(data.ThrottleDelaySeconds.toString(), 10);
    config.SegmentKeepSeconds = parseInt(data.SegmentKeepSeconds.toString(), 10);
    config.HardwareDecodingCodecs = CODECS
        .map(c => c.codec)
        .filter(c => data[c]?.toString() === 'on');
    config.EnableDecodingColorDepth10Hevc = data.EnableDecodingColorDepth10Hevc.toString() === 'on';
    config.EnableDecodingColorDepth10Vp9 = data.EnableDecodingColorDepth10Vp9.toString() === 'on';
    config.EnableDecodingColorDepth10HevcRext = data.EnableDecodingColorDepth10HevcRext.toString() === 'on';
    config.EnableDecodingColorDepth12HevcRext = data.EnableDecodingColorDepth12HevcRext.toString() === 'on';
    config.EnableEnhancedNvdecDecoder = data.EnableEnhancedNvdecDecoder.toString() === 'on';
    config.PreferSystemNativeHwDecoder = data.PreferSystemNativeHwDecoder.toString() === 'on';
    config.EnableIntelLowPowerH264HwEncoder = data.EnableIntelLowPowerH264HwEncoder.toString() === 'on';
    config.EnableIntelLowPowerHevcHwEncoder = data.EnableIntelLowPowerHevcHwEncoder.toString() === 'on';
    config.EnableHardwareEncoding = data.EnableHardwareEncoding.toString() === 'on';
    config.AllowHevcEncoding = data.AllowHevcEncoding.toString() === 'on';
    config.AllowAv1Encoding = data.AllowAv1Encoding.toString() === 'on';
                    ...
                    <Form method='POST'>
                        <Stack spacing={3}>
                            ...
                            <TextField
                                ...
                                style={hardwareAccelType != 'vaapi' ? { display: 'none' } : undefined}
                            />
                            ...
                            <TextField
                                ...
                                style={hardwareAccelType != 'qsv' ? { display: 'none' } : undefined}
                            />

                            <FormControl
                                variant='standard'
                                style={hardwareAccelType == 'none' ? { display: 'none' } : undefined}
                            >
                                ...
                                <FormGroup>
                                    {CODECS.map(codec => (
                                        <FormControlLabel
                                            key={codec.name}
                                            label={codec.name}
                                            style={!codec.types.includes(hardwareAccelType) ? { display: 'none' } : undefined}
                                            control={
                                                <Checkbox
                                                    name={codec.codec}
                                    ...
                                    <FormControlLabel
                                        label={'HEVC 10bit'}
                                        style={!HEVC_VP9_HW_DECODING_TYPES.includes(hardwareAccelType) ? { display: 'none' } : undefined}
                                        control={
                                            <Checkbox
                                    ...
                                    <FormControlLabel
                                        ...
                                        style={!HEVC_REXT_DECODING_TYPES.includes(hardwareAccelType) ? { display: 'none' } : undefined}
                                        control={
                                            <Checkbox
                            ...
                            <FormControl style={hardwareAccelType != 'nvenc' ? { display: 'none' } : undefined}>
                                <FormControlLabel
                            ...
                            <FormControl style={hardwareAccelType != 'qsv' ? { display: 'none' } : undefined}>
                                <FormControlLabel
