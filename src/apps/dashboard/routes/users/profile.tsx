...
function onSaveComplete() {
    Dashboard.navigate('/dashboard/users').catch((err) => {
...
const UserEdit = () => {
    ...
    const loadDeleteFolders = useCallback(
        (page: HTMLDivElement, user: UserDto, mediaFolders: BaseItemDto[]) => {
            window.ApiClient.getJSON(
    ...
    const loadUser = useCallback(
        (user: UserDto) => {
            ...
            window.ApiClient.getJSON(window.ApiClient.getUrl('Auth/Providers'))
                .then(function (providers) {
                    loadAuthProviders(page, user, providers);
                })
                .catch((err) => {
                    console.error(
                        '[useredit] failed to fetch auth providers',
                        err
                    );
                });
            window.ApiClient.getJSON(
                window.ApiClient.getUrl('Auth/PasswordResetProviders')
            )
                .then(function (providers) {
                ...
                .catch((err) => {
                    console.error(
                        '[useredit] failed to fetch password reset providers',
                        err
                    );
                });
            window.ApiClient.getJSON(
                ...
                .catch((err) => {
                    console.error(
                        '[useredit] failed to fetch media folders',
                        err
                    );
                });
            ...
            ).checked = !!user.Policy?.EnableCollectionManagement;
            (
                page.querySelector(
                    '.chkEnableSubtitleManagement'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.EnableSubtitleManagement;
            (
                page.querySelector(
                    '.chkRemoteControlSharedDevices'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.EnableSharedDeviceControl;
            (
                page.querySelector(
                    '.chkEnableRemoteControlOtherUsers'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.EnableRemoteControlOfOtherUsers;
            (
                page.querySelector('.chkEnableDownloading') as HTMLInputElement
            ).checked = !!user.Policy?.EnableContentDownloading;
            (
                page.querySelector('.chkManageLiveTv') as HTMLInputElement
            ).checked = !!user.Policy?.EnableLiveTvManagement;
            (
                page.querySelector('.chkEnableLiveTvAccess') as HTMLInputElement
            ).checked = !!user.Policy?.EnableLiveTvAccess;
            (
                page.querySelector(
                    '.chkEnableMediaPlayback'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.EnableMediaPlayback;
            (
                page.querySelector(
                    '.chkEnableAudioPlaybackTranscoding'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.EnableAudioPlaybackTranscoding;
            (
                page.querySelector(
                    '.chkEnableVideoPlaybackTranscoding'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.EnableVideoPlaybackTranscoding;
            (
                page.querySelector(
                    '.chkEnableVideoPlaybackRemuxing'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.EnablePlaybackRemuxing;
            (
                page.querySelector(
                    '.chkForceRemoteSourceTranscoding'
                ) as HTMLInputElement
            ).checked = !!user.Policy?.ForceRemoteSourceTranscoding;
            (
                page.querySelector('.chkRemoteAccess') as HTMLInputElement
            ).checked =
                user.Policy?.EnableRemoteAccess == null ||
            ...
            (
                ...
                user.Policy?.RemoteClientBitrateLimit &&
                user.Policy?.RemoteClientBitrateLimit > 0
                    ? (
            ...
            ).value = String(user.Policy?.LoginAttemptsBeforeLockout) || '-1';
            (
                page.querySelector('#txtMaxActiveSessions') as HTMLInputElement
            ).value = String(user.Policy?.MaxActiveSessions) || '0';
            (
                page.querySelector('#selectSyncPlayAccess') as HTMLSelectElement
            ).value = String(user.Policy?.SyncPlayAccess);
            loading.hide();
        },
    ...
    useEffect(() => {
        ...
        const saveUser = (user: UserDto) => {
            ...
            window.ApiClient.updateUser(user)
                .then(() =>
                    window.ApiClient.updateUserPolicy(
                        user.Id || '',
                        user.Policy || {
        ...
        window.ApiClient.getNamedConfiguration('network')
            .then(function (config) {
                ...
                ).classList.toggle('hide', !config.EnableRemoteAccess);
            })
            .catch((err) => {
