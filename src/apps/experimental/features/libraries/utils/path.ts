 ...
 */
export const isLibraryPath = (path: string) => LibraryRoutes.some(route => route.path === path);
...
export const getDefaultViewIndex = (path: string, libraryId?: string | null) => {
    ...
    return views.find(view => view.view === defaultView)?.index ?? views.find(view => view.isDefault)?.index ?? 0;
};
