define(["dom", "layoutManager", "inputManager", "connectionManager", "events", "viewManager", "libraryBrowser", "appRouter", "apphost", "playbackManager", "browser", "globalize", "paper-icon-button-light", "material-icons", "scrollStyles", "flexStyles"], function (dom, layoutManager, inputManager, connectionManager, events, viewManager, libraryBrowser, appRouter, appHost, playbackManager, browser, globalize) {
  ...
  function onHeaderUserButtonClick(e__q) {
    Dashboard.navigate("mypreferencesmenu.html");
  }

  function onSettingsClick(e__w) {
  ...
  function onMainDrawerSelect(e__e) {
    if (navDrawerInstance.isVisible) {
      onMainDrawerOpened();
    } else {
  ...
  function updateDashboardMenuSelectedItem() {
    ...
    for (var i__r = 0, length = links.length; i__r < length; i__r++) {
      var link = links[i__r];
      var selected = false;
  ...
  function addPluginPagesToMainMenu(links, pluginItems, section) {
    for (var i__t = 0, length = pluginItems.length; i__t < length; i__t++) {
      var pluginItem = pluginItems[i__t];
  ...
  function getToolsMenuHtml(apiClient) {
    return getToolsMenuLinks(apiClient).then(function (items) {
      ...
      for (menuHtml += '<div class="drawerContent">', i__y = 0, length = items.length; i__y < length; i__y++) {
        item = items[i__y];
  ...
  function getUserViews(apiClient, userId) {
    return apiClient.getUserViews({}, userId).then(function (result) {
      ...
      for (var i__u = 0, length = items.length; i__u < length; i__u++) {
        var view = items[i__u];
      ...
      getUserViews(apiClient, userId).then(function (result) {
        ...
        html += items.map(function (i__o) {
          ...
          if ("channels" == i__o.CollectionType) {
            ...
            if ("livetv" == i__o.CollectionType) {
              itemId = "livetv";
            }
          }

          if ("photos" == i__o.CollectionType) {
            icon = "photo_library";
          } else {
            if ("music" == i__o.CollectionType || "musicvideos" == i__o.CollectionType) {
              icon = "library_music";
            } else {
              if ("books" == i__o.CollectionType) {
                icon = "library_books";
              } else {
                if ("playlists" == i__o.CollectionType) {
                  icon = "view_list";
                } else {
                  if ("games" == i__o.CollectionType) {
                    icon = "games";
                  } else {
                    if ("movies" == i__o.CollectionType) {
                      icon = "video_library";
                    } else {
                      if ("channels" == i__o.CollectionType || "Channel" == i__o.Type) {
                        icon = "videocam";
                      } else {
                        if ("tvshows" == i__o.CollectionType) {
                          icon = "tv";
                        } else {
                          if ("livetv" == i__o.CollectionType) {
                            icon = "live_tv";
          ...
          return '<a is="emby-linkbutton" data-itemid="' + itemId + '" class="lnkMediaFolder navMenuOption" href="' + getItemHref(i__o, i__o.CollectionType) + '"><i class="md-icon navMenuOptionIcon">' + icon + '</i><span class="sectionName navMenuOptionText">' + i__o.Name + "</span></a>";
        }).join("");
        libraryMenuOptions.innerHTML = html;
        var elem = libraryMenuOptions;
        var sidebarLinks = elem.querySelectorAll(".navMenuOption");

        for (var i__i = 0, length = sidebarLinks.length; i__i < length; i__i++) {
          sidebarLinks[i__i].removeEventListener("click", onSidebarLinkClick);
          sidebarLinks[i__i].addEventListener("click", onSidebarLinkClick);
        }
      });
    }
  }
  ...
  function onMainDrawerClick(e__p) {
    if (dom.parentWithTag(e__p.target, "A")) {
  ...
  function updateLibraryNavLinks(page) {
    ...
    for (i__a = 0, length = elems.length; i__a < length; i__a++) {
      var lnkMediaFolder = elems[i__a];
      ...
      if (isChannelsPage && "channels" == itemId) {
        ...
        if (isLiveTvPage && "livetv" == itemId) {
          lnkMediaFolder.classList.add("navMenuOption-selected");
        } else {
          if (isEditorPage && "editor" == itemId) {
            lnkMediaFolder.classList.add("navMenuOption-selected");
          } else {
            if (isMySyncPage && "manageoffline" == itemId && -1 != window.location.href.toString().indexOf("mode=download")) {
              lnkMediaFolder.classList.add("navMenuOption-selected");
            } else {
              if (isMySyncPage && "syncotherdevices" == itemId && -1 == window.location.href.toString().indexOf("mode=download")) {
                lnkMediaFolder.classList.add("navMenuOption-selected");
              } else {
                if (id && itemId == id) {
                  lnkMediaFolder.classList.add("navMenuOption-selected");
  ...
  var currentPageType;
  pageClassOn("pagebeforeshow", "page", function (e__s) {
    if (!this.classList.contains("withTabs")) {
      LibraryMenu.setTabs(null);
    }
  });
  pageClassOn("pageshow", "page", function (e__d) {
    var page = this;
  ...
  events.on(connectionManager, "localusersignedin", function (e__f, user) {
