...
const RELEASE_THRESHOLD_HYSTERESIS = 0.6;
const REPEAT_INITIAL_DELAY_MS = 400;
const REPEAT_INTERVAL_MS = 100;
const PRESS_CONSECUTIVE_FRAMES = 1;
...
function handleInitialDown(keyName, keyCode, repeatsAllowed, nowTs) {
    ...
    if (repeatsAllowed) {
        state.nextRepeatAt.set(keyName, nowTs + REPEAT_INITIAL_DELAY_MS);
    } else {
...
function handleRepeat(keyName, keyCode, nowTs) {
    ...
    const producedEvent = dispatchKeyboardEvent('keydown', keyName, keyCode);
    if (producedEvent) state.nextRepeatAt.set(keyName, nowTs + REPEAT_INTERVAL_MS);
    return { event: producedEvent, logicalPressed: true };
}
...
function axisReleased(value, positive = true) {
    return positive ? value < RELEASE_THRESHOLD_HYSTERESIS : value > -RELEASE_THRESHOLD_HYSTERESIS;
}
