...
export function showSortMenu(options: any): void {
    Promise.all([
        ...
        html +=
            '<h2 style="margin:0 0 .5em;">' + globalize.translate('HeaderSortBy') + '</h2><div>';

        options.items.forEach((item: any) => {
            const radioValue = item.id.replace(',', '_');
            const isChecked =
                (options.query.SortBy || '').replace(',', '_') === radioValue ? ' checked' : '';
            html +=
        ...
        html +=
            '</div><h2 style="margin: 1em 0 .5em;">' +
            globalize.translate('HeaderSortOrder') +
            '</h2><div>';
