...
function loadChildrenOfRootNode(page: Document, scope, callback) {
...
function loadMediaFolders(page: Document, scope, openItems, callback) {
...
function initializeTree(page: Document, currentUser, openItems, selectedId ?: string) {
    Promise.all([
        import('jstree'),
...
function initializeTreeInternal(page: Document, currentUser, openItems, selectedId) {
    nodesToLoad = [];
    selectedNodeId = null;
    $.jstree.destroy();
...
function loadNodesToLoad(page: Document, node) {
...
function updateEditorNode(page: Document, item) {
