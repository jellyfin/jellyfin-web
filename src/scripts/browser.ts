...
function hasKeyboard(browser: BrowserType) {
    if (browser.touch) {
        return true;
    }
...
function web0sVersion(browser: BrowserType) {
...
let _supportsCssAnimation: boolean | undefined = undefined;
let _supportsCssAnimationWithPrefix: boolean | undefined = undefined;
...
function uaMatch(ua: string) {
    ...
    let browser: BrowserTypeStrings = match[1] || '';
...
export type BrowserType = typeof browser;
export type BrowserTypeStrings = Exclude<keyof BrowserType, 'version' | 'versionMajor' | 'platform' | 'supportsCssAnimation' | 'iOSVersion' | 'tizenVersion' | 'web0sVersion'>;
