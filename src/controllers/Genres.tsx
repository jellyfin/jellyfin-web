...
export const Genres: React.FC = () => {
    const params = useParams<{ topParentId?: string; genreType?: string }>();
    ...
    const {
        ...
        hasNextPage,
        hasPreviousPage
    } = usePagination(`genres-${topParentId}-${genreType}`);
    ...
    const { data, isLoading, isError, error, refetch } = useQuery({
        ...
        queryFn: async () => {
            ...
            return itemsApi.getItems(topParentId, {
                ...
                limit: pageSize,
                recursive: true,
                includeItemTypes: ['MusicGenre', 'Genre'],
            });
        },
    ...
    return (
        <div className={styles.container}>
            <div className={styles.headerRow}>
                <Heading.H3>
                    {genreType === 'music' ? 'Music Genres' : 'Genres'}
                </Heading.H3>
                <div className={styles.chipGroup}>
                    <Chip variant={genreType === 'music' ? 'primary' : 'neutral'}>
                        Music
                    </Chip>
                    <Chip variant={genreType === 'movies' ? 'primary' : 'neutral'}>
                        Movies
                    </Chip>
                    <Chip variant={genreType === 'tv' ? 'primary' : 'neutral'}>
                        TV
                    </Chip>
                </div>
            </div>
