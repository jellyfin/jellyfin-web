...
function renderName(item, container, context) {
    ...
    if (item.AlbumArtists) {
        ...
        parentNameHtml.push(
            `<a style="color:inherit;" class="button-link itemAction" is="emby-linkbutton" href="#" data-action="link" data-id="${item.SeriesId}" data-serverid="${item.ServerId}" data-type="Series" data-isfolder="true">${escapeHtml(item.SeriesName)}</a>`
        );
    ...
    if (item.SeriesName && item.Type === 'Season') {
        ...
        parentNameHtml.push(
            `<a style="color:inherit;" class="button-link itemAction" is="emby-linkbutton" href="#" data-action="link" data-id="${item.SeasonId}" data-serverid="${item.ServerId}" data-type="Season" data-isfolder="true">${escapeHtml(item.SeasonName)}</a>`
        );
        ...
        parentNameHtml.push(
            `<a style="color:inherit;" class="button-link itemAction" is="emby-linkbutton" href="#" data-action="link" data-id="${item.AlbumId}" data-serverid="${item.ServerId}" data-type="MusicAlbum" data-isfolder="true">${escapeHtml(item.Album)}</a>`
        );
    } else if (item.Album) {
...
function reloadFromItem(instance, page, params, item, user) {
    ...
    page.querySelector('.detailPagePrimaryContainer').classList.add(
        'detailRibbon'
    );
...
function renderMoreFromArtist(view, item, apiClient) {
    ...
    if (section) {
        ...
        if (item.Type === 'MusicArtist') {
            ...
            query.ContributingArtistIds = item.AlbumArtists.map(
                (artist) => artist.Id
            ).join(',');
        }
...
function renderChildren(page, item) {
    ...
    promise.then(function (result) {
        ...
        let scrollX = false;
        let isList = false;
        const childrenItemsContainer = page.querySelector(
    ...
    if (item.Type == 'Season') {
        page.querySelector('#childrenTitle').innerHTML =
        ...
        page.querySelector('#childrenTitle').innerHTML =
            globalize.translate('HeaderTracks');
    } else {
        page.querySelector('#childrenTitle').innerHTML =
            globalize.translate('Items');
    }

    if (item.Type == 'MusicAlbum' || item.Type == 'Season') {
        page.querySelector('.childrenSectionHeader').classList.add('hide');
        page.querySelector('#childrenCollapsible').classList.add(
