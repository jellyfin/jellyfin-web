...
function loadImages(element, apiClient, parentId) {
    return apiClient.getItems(apiClient.getCurrentUserId(), {
        ...
        if (items.length > 0) {
            element.querySelector(".tvEpisodesCard .cardImage").style.backgroundImage = "url('" + imagehelper.getbackdropImageUrl(items[0], imgOptions) + "')";
        }
        if (items.length > 1) {
            element.querySelector(".allSeriesCard .cardImage").style.backgroundImage = "url('" + imagehelper.getbackdropImageUrl(items[1], imgOptions) + "')";
        }
        return;
    });
}
