...
interface UseAudioAnalysisReturn {
    ...
    analyzeFromSamples: (trackId: string, samples: Float32Array, sampleRate: number, duration: number) => Promise<AudioAnalysisResult>;
    ...
    analyzeNextTrack: (currentTrackId: string, nextTrackId: string, nextAudioBuffer: AudioBuffer) => Promise<TransitionSuggestion>;
}
...
interface WasmAnalyzer {
    AudioAnalyzer: new (fftSize?: number) => {
        ...
        classifyGenre(features: WasmFeatures): { primary_genre: string; genre_confidence: number; genre_scores: number[]; sub_genres: string[] };
        suggestTransition(current: WasmFeatures, next: WasmFeatures, structure: WasmStructure): {
...
function convertWasmGenre(genre: { primary_genre: string; genre_confidence: number; genre_scores: number[]; sub_genres: string[] }) {
    return {
...
export function useAudioAnalysis(): UseAudioAnalysisReturn {
    ...
    const { saveAnalysis, getAnalysis, getTransitionSuggestion, saveTransitionCandidate, getCompatibleTracks: getCompatible } =
        useAnalysisStore();
    ...
    const analyzeFromSamples = useCallback(async (
        trackId: string,
    ...
    const analyze = useCallback(async (trackId: string, audioBuffer: AudioBuffer): Promise<AudioAnalysisResult> => {
        const channelData = audioBuffer.getChannelData(0);
        return analyzeFromSamples(trackId, channelData, audioBuffer.sampleRate, audioBuffer.duration);
    }, [analyzeFromSamples]);

    const analyzeNextTrack = useCallback(async (
        ...
        const currentFeatures = currentAnalysis?.features ? {
            ...
            bpm: 120, bpm_confidence: 0.5, key: 'C Major', key_confidence: 0.5,
            camelot_key: '8B', energy: 0.5, loudness: -20, spectral_centroid: 2000,
            spectral_rolloff: 8000, spectral_flux: 0.1, zero_crossing_rate: 0.05,
            rms_energy: 0.1, peak_frequency: 440, dynamic_range: 20,
            attack_time: 0.1, decay_time: 0.5, brightness: 0.2, warmth: 0.5, roughness: 0.3
        };
        ...
        const currentStructure = currentAnalysis?.structure ? {
            ...
            intro_start: 0, intro_end: 30, outro_start: 270, outro_end: 300,
            drops: [], breakdowns: [], buildups: [], energy_profile: [], section_count: 3
        };
    ...
    const getTransition = useCallback((currentTrackId: string, nextTrackId: string) => {
        return getTransitionSuggestion(currentTrackId, nextTrackId);
    }, [getTransitionSuggestion]);

    const getCompatibleTracks = useCallback((trackId: string, limit?: number) => {
